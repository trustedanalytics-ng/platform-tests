...
@ddt.ddt
class PsqlSpecificTests(unittest.TestCase):
    ...
    def test_cannot_post_null_to_not_null_columns(self, values):
        self.assertRaises(IntegrityError, self.db.add_row, table_name=self.table_name, values_dict=values)

    def test_cannot_put_string_to_integer_type_column(self):
        updated_values = {COL_NAMES[0]: "sss"}
        self.assertRaises(DataError, self.db.update_row, self.table_name, 1, updated_values)

    def test_cannot_put_bool_to_integer_column(self):
        updated_values = {COL_NAMES[2]: False}
