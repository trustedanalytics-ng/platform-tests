...
def api_get_metric(client=None, panel_id=None, metrics_level=None):
    ...
    dashboard_queries = {
        ...
        "7": "tap_datacatalog_counts{component='datasets'}"
    }
    if metrics_level == "platform":
        step = "60"
        headers_referer = "{}/dashboard-solo/db/platform-dashboard?panelId={}&from=now-1h&to=now".format(client.url,
                                                                                                         panel_id)
        dashboard_queries = {
    ...
    query = dashboard_queries.get(panel_id)
