...
def api_get_datasets(org_guid_list=None, query="", filters=(), size=12, time_from=0, only_private=False,
                     only_public=False, client: HttpClient = None):
    """GET /rest/datasets"""
    query_params = {
    ...
    client = client or HttpClientFactory.get(ConsoleDataCatalogConfigurationProvider.get())
...
def api_get_dataset(entry_id, client: HttpClient = None):
    """GET /rest/datasets/{entry_id}"""
    client = client or HttpClientFactory.get(ConsoleDataCatalogConfigurationProvider.get())
    return client.request(
        method=HttpMethod.GET,
        path="/{}".format(entry_id),
        msg="PLATFORM: get data set"
    )


def api_delete_dataset(entry_id, client: HttpClient = None):
...
def api_update_dataset(entry_id, creation_time=None, target_uri=None, category=None, format=None,
                       ...
                       title=None, client: HttpClient = None):
