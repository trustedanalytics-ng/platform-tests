...
class TestSecrets:
    ...
    def test_1_create_secret(self, test_org):
        step("Create new secret")
        self.__class__.secret = KubernetesSecret.create(org_guid=test_org.guid)
        step("Get secret and check both are the same")
        secret = KubernetesSecret.get(org_guid=test_org.guid, key_id=self.secret.key_id)
        assert self.secret == secret

    def test_2_update_secret(self, test_org):
        ...
        new_secret = KubernetesSecret.get(org_guid=test_org.guid, key_id=self.secret.key_id)
    ...
    def test_3_delete_secret(self, test_org):
        step("Delete secret")
        self.secret.delete()
        ...
        assertions.assert_raises_http_exception(KubernetesBrokerHttpStatus.CODE_INTERNAL_SERVER_ERROR, error_message,
                                                KubernetesSecret.get, org_guid=test_org.guid, key_id=self.secret.key_id)
...
class TestSecretsNegativeCases:
    ...
    def test_cannot_create_secret_with_existing_id(self, test_org):
        step("Create secret")
        secret = KubernetesSecret.create(org_guid=test_org.guid)
        ...
        assertions.assert_raises_http_exception(KubernetesBrokerHttpStatus.CODE_INTERNAL_SERVER_ERROR, error_message,
                                                KubernetesSecret.create, org_guid=test_org.guid, key_id=secret.key_id)

    def test_cannot_create_secret_with_wrong_id(self, test_org):
        ...
        assertions.assert_raises_http_exception(KubernetesBrokerHttpStatus.CODE_INTERNAL_SERVER_ERROR, error_message,
                                                KubernetesSecret.create, org_guid=test_org.guid, key_id=key_id)

    def test_cannot_get_secret_with_not_existing_id(self, test_org):
        ...
        assertions.assert_raises_http_exception(KubernetesBrokerHttpStatus.CODE_INTERNAL_SERVER_ERROR, error_message,
                                                KubernetesSecret.get, org_guid=test_org.guid, key_id=key_id)

    def test_cannot_update_secret_with_not_existing_id(self, test_org):
        step("Try to update secret with not existing id")
        secret = KubernetesSecret(org_guid=test_org.guid, key_id=KubernetesSecret.generate_key(key_len=20),
                                  username="test_username", password="test_password")
        error_message = KubernetesBrokerHttpStatus.MSG_SECRET_NOT_FOUND.format(secret.key_id)
        assertions.assert_raises_http_exception(KubernetesBrokerHttpStatus.CODE_INTERNAL_SERVER_ERROR, error_message,
                                                secret.update)

    def test_cannot_delete_secret_with_not_existing_id(self, test_org):
