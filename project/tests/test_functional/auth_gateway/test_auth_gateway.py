...
from modules.tap_object_model import AuthGatewayUser, User, Transfer
...
@pytest.mark.skip(reason="DPNG-11790 Call Auth-Gateway synchronization endpoint at the end of apps installation.")
...
class TestAuthGateway:
    ...
    def test_add_user(self, test_org, context):
        ...
        transfer = Transfer.api_create_by_file_upload(context, test_org.guid, Urls.test_transfer_link, client=client)
        transfer.ensure_finished(self)
    ...
    def test_remove_user(self, context, test_org):
        ...
        ag_user = AuthGatewayUser.get_user_state(org_guid=test_org.guid, user_id=user_to_delete.guid)
        assert ag_user.is_synchronized, "User was not synchronized"
        assert user_to_delete.login() is None, "User was not removed from platform"
