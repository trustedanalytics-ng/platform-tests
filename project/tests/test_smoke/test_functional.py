...
def test_create_and_delete_hdfs_service_user_directory(core_org, core_space, core_marketplace):
    step("Create {} instance with {} plan".format(ServiceLabels.HDFS, ServicePlan.CREATE_USER_DIRECTORY))
    hdfs_service = next((s for s in core_marketplace if s.label == ServiceLabels.HDFS), None)
    assert hdfs_service is not None, "{} service was not found".format(ServiceLabels.HDFS)
    create_directory_plan_guid = next((p["guid"] for p in hdfs_service.service_plans
                                       if p["name"] == ServicePlan.CREATE_USER_DIRECTORY), None)
    assert create_directory_plan_guid is not None,\
        "Plan with name {} for service {} not found".format(ServicePlan.CREATE_USER_DIRECTORY, ServiceLabels.HDFS)
    instance_1 = ServiceInstance.api_create(org_guid=core_org.guid, space_guid=core_space.guid,
    ...
    get_directory_plan_guid = next((p["guid"] for p in hdfs_service.service_plans if p["name"] == ServicePlan.GET_USER_DIRECTORY), None)
    assert create_directory_plan_guid is not None,\
        "Plan with name {} for service {} not found".format(ServicePlan.GET_USER_DIRECTORY, ServiceLabels.HDFS)
    instance_2 = ServiceInstance.api_create(org_guid=core_org.guid, space_guid=core_space.guid,
                                            service_label=ServiceLabels.HDFS, service_plan_guid=get_directory_plan_guid,
                                            params={"uri": uri})
    instance_2.ensure_created()

    step("Delete {} instance with plan {}".format(ServiceLabels.HDFS, ServicePlan.GET_USER_DIRECTORY))
    instance_2.api_delete()
    step("Delete key for {} {}".format(ServiceLabels.HDFS, ServicePlan.CREATE_USER_DIRECTORY))
    key.api_delete()
    step("Delete {} instance with plan {}".format(ServiceLabels.HDFS, ServicePlan.CREATE_USER_DIRECTORY))
