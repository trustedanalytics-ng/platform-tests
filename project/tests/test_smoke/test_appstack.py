...
@priority.high
class TestSmokeTrustedAnalytics:
    ...
    def expected_appstack(self):
        step("Retrieve content of appstack.yml file")
        appstack_yml = get_appstack_yml(TapGitHub.intel_data)
        step("Retrieve expected app, service, and broker names from the file")
        ...
        self.expected_app_names = {app_info["name"] for app_info in appstack_yml["apps"]
                                   if should_be_started(app_info) and should_exist(app_info)}
        self.expected_upsi_names = {app_info["name"] for app_info in appstack_yml["user_provided_services"]
                                    if should_exist(app_info)}
        self.expected_broker_names = {app_info["name"] for app_info in appstack_yml["brokers"]
                                      if should_exist(app_info)}

    @classmethod
    @pytest.fixture(scope="class", autouse=True)
    def appstack(self, core_space):
        step("Retrieve apps, services, and brokers present in cf")
        self.cf_apps = Application.cf_api_get_list_by_space(core_space.guid)
        self.cf_upsi = [s for s in Upsi.cf_api_get_list() if s.space_guid == core_space.guid]
        self.cf_brokers = ServiceBroker.cf_api_get_list(core_space.guid)

        step("Retrieve apps and services present on the Platform")
        self.platform_apps = Application.api_get_list(core_space.guid)
        self.platform_instances = ServiceInstance.api_get_list(core_space.guid)

    def test_all_required_apps_are_present_in_cf(self):
        step("Check that all expected apps are present in cf")
        cf_app_names = {a.name for a in self.cf_apps}
        missing_apps = self.expected_app_names - cf_app_names
        assert missing_apps == set(), "Apps missing in cf"

    def test_all_required_apps_are_running_in_cf(self):
        step("Check that all expected apps have running instances in cf")
        ...
        assert apps_not_running == set(), "Apps with no running instances in cf"
    ...
    def test_apps_have_the_same_details_in_cf_and_on_platform(self):
        ...
        for app in only_expected_platform_apps:
            step("Check that details of app {} are the same in cf and on the Platform".format(app.name))
    ...
    def test_spring_services_dont_expose_sensitive_endpoints(self):
        ...
        for url in [a.urls[0] for a in self.platform_apps
            ...
            try:
                ...
                continue
            step("Check that the sensitive endpoints are not enabled.")
            enabled_endpoints = []
